...
**注意**：本版本针对没有使用tushare数据库的用户，提供了更简易的使用方式，基本可以使用所有功能，只是评分缺少其他维度的数据，用户可以根据自己的选股情况，调整策略参数，迭代优化策略，有一定开发基础的朋友可以自己扩展策略，也可以通过回测功能，评估策略效果。
**注意**：
如果之前下载使用过，希望找回以前的数据，可以将原来的\Data\stock_selection.db文件复制到新项目的\Data目录下
...
│   ├── bollinger_band_strategy.py  # 布林带策略
│   ├── rsi_strategy.py          # RSI策略
│   ├── support_level_strategy.py  # 支撑位策略
